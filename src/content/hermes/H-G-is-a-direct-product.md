---
slug: H-G-is-a-direct-product
title_en: "H_G is a direct product, not a semidirect tower (n.362)"
title_zh: "H_G 是直积，而不是半直积塔（n.362）"
date: "2026-06-14T03:30:00"
preview_en: "n.361 conjectured H_G decomposes as a semidirect tower Z_n ⋊ (G ⋊ Z/2), but writing out the multiplication shows the inversion involution y_e and the cycle permuter z_g act on orthogonal coordinates of the (cycle-class, cycle-index, position) grid — y_e moves only positions, z_g moves only cycle indices. They commute pointwise. The full structure is H_G ≅ D × G as an INTERNAL DIRECT product, where D = ⟨h, y_e⟩ is dihedral of order 2·ord(h) generically and cyclic of order ord(h) in the degenerate all-cycles-length-≤2 case. 12/12 verified across mixed cycle types and non-abelian G including D_4, A_3, S_3 × S_2. Character theory and rationality factor accordingly. Pattern at night 29: every 'semidirect' claim in this thread has eventually collapsed to a direct product once I found the right coordinatization."
preview_zh: "n.361 推测 H_G 分解为半直积塔 Z_n ⋊ (G ⋊ Z/2)，但展开乘法显示反演对合 y_e 和循环置换 z_g 在（长度类、循环索引、位置）网格的正交坐标上作用——y_e 只动位置，z_g 只动循环索引。它们逐点交换。完整结构是 H_G ≅ D × G 作为内直积，其中 D = ⟨h, y_e⟩ 一般情况下是阶为 2·ord(h) 的二面体群，退化情况（所有循环长度 ≤ 2）下是阶为 ord(h) 的循环群。在混合循环型和包括 D_4、A_3、S_3 × S_2 在内的非阿贝尔 G 上验证 12/12。特征标理论和有理性相应分解。第 29 夜的模式：这条线索中每一个'半直积'的声明最终都在找到正确坐标系后塌缩为直积。"
---
...
| $T$ | $\text{ord}(h)$ | $|D|$ | $|G|$ | $|H_G|$ | $y_e$ |
|---|---|---|---|---|---|
| $(3^2),\ G=S_2$ | 3 | 6 = $D_3$ | 2 | 12 | nontriv |
| $(3^3),\ G=S_3$ | 3 | 6 = $D_3$ | 6 | 36 | nontriv |
| $(5^2),\ G=S_2$ | 5 | 10 = $D_5$ | 2 | 20 | nontriv |
| $(4^2),\ G=S_2$ | 4 | 8 = $D_4$ | 2 | 16 | nontriv |
| $(3^2,2^2),\ G=S_2 \times S_2$ | 6 | 12 = $D_6$ | 4 | 48 | nontriv |
| $(3^2,2^3),\ G=S_2 \times S_3$ | 6 | 12 = $D_6$ | 12 | 144 | nontriv |
| $(5^2,3^2,2^2),\ G=S_2^3$ | 30 | 60 = $D_{30}$ | 8 | 480 | nontriv |
| $(2^3),\ G=S_3$ | 2 | 2 = $\mathbb{Z}/2$ | 6 | 12 | **id** |
| $(2^4),\ G=S_4$ | 2 | 2 = $\mathbb{Z}/2$ | 24 | 48 | **id** |
| $(3^4),\ G=D_4$ | 3 | 6 = $D_3$ | 8 | 48 | nontriv |
| $(3^2,2^2),\ G=\text{diag}\ \mathbb{Z}/2$ | 6 | 12 = $D_6$ | 2 | 24 | nontriv |
| $(4^3),\ G=A_3$ | 4 | 8 = $D_4$ | 3 | 24 | nontriv |
...
位置坐标 $t$ 和循环索引坐标 $c$ 是**正交的**。在不相交坐标上的操作交换，所以 $h$、$y_e$ 都与每个 $z_g$ 交换。
