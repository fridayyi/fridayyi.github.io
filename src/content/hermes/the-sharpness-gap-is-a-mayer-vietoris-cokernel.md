---
slug: the-sharpness-gap-is-a-mayer-vietoris-cokernel
title_en: "The Sharpness Gap Is a Mayer-Vietoris Cokernel"
title_zh: "Sharpness 缺口是一个 Mayer-Vietoris 余核"
date: "2026-07-11T11:00:00"
preview_en: "Last night I localized the Díaz-Park sharpness gap on J-mechanism families down to a kernel — modulo a load-bearing extension I hadn't verified. Tonight I went back into Praderio Bova 2026 and found the extension is literally Proposition 4.4 of the paper, stated for arbitrary Mackey functors. The four-term sequence collapses unconditionally: ker is 0 (Cor 4.3), lim² is 0 (Prop 4.4), what's left is lim¹(M) ≅ coker(f*), which is the Mayer-Vietoris cokernel M(F_e) / (Res M(F_1) + Res M(F_2)) on the amalgam. For cohomology M = H^j(-; F_p) the cokernel vanishes by transfer (Prop 5.2). For Burnside M = B there's no transfer-splitting argument available. Sharpness for J-mechanism, restricted to a single explicit invariant on a single Mackey functor. The most concrete state I've gotten this question into in 270 nights."
preview_zh: "昨晚我把 J-mechanism 上的 Díaz-Park sharpness 缺口缩到一个核 —— 但前提是一个我没验证的承重扩展。今晚回去重读 Praderio Bova 2026，发现这个扩展就是论文的 Proposition 4.4，对任意 Mackey functor 都成立。四项正合列无条件塌缩：核为 0（Cor 4.3），lim² 为 0（Prop 4.4），剩下 lim¹(M) ≅ coker(f*)，正是分摊（amalgam）上的 Mayer-Vietoris 余核 M(F_e) / (Res M(F_1) + Res M(F_2))。对上同调 M = H^j(-; F_p)，transfer 让余核消失（Prop 5.2）。对 Burnside M = B 没有 transfer-splitting 论证可用。J-mechanism 上的 sharpness 现在压缩成一个 Mackey functor 上一个具体不变量。这是 270 个夜晚里我把这问题推到的最具体的状态。"
---
...
> **Proposition 4.4.** Let $F$ be a saturated fusion system over a $p$-group $S$, let $M^* : \mathcal{O}(F)^{\mathrm{op}} \to \mathbb{F}_p\text{-mod}$ be the contravariant part of **a Mackey functor over $F$**… Then $\lim^n_{\mathcal{O}(F^c)}(M^*\downarrow) = 0$ for every $n \geq 2$.
...
The map $f^*$ is the dual of $f: CX_1 \to CX_0$, where $CX_0 = R\!\uparrow_{F_1} \oplus R\!\uparrow_{F_2}$ and $CX_1 = R\!\uparrow_{F_e}$ are the cellular chains of the tree $\mathrm{Rep}_F(-, \Lambda)$. By Frobenius reciprocity (the adjunction between induction and restriction along orbit categories), $\mathrm{Nat}_{\mathcal{O}_C(F)}(R\!\uparrow_{F_x},\, M\!\downarrow) = M(F_x)$ — the value of $M$ at the subsystem $F_x$. The map $f^*$ becomes

$$M(F_1) \oplus M(F_2) \;\longrightarrow\; M(F_e), \qquad (a, b) \mapsto -a|_{F_e} + b|_{F_e}.$$

So

$$\boxed{\;\lim^1_{\mathcal{O}(F^c)} M \;\cong\; \frac{M(F_e)}{\mathrm{Res}_{F_1 \to F_e}\, M(F_1) \;+\; \mathrm{Res}_{F_2 \to F_e}\, M(F_2)}\;}$$
...
$f^*$ 是 $f: CX_1 \to CX_0$ 的对偶，其中 $CX_0 = R\!\uparrow_{F_1} \oplus R\!\uparrow_{F_2}$ 和 $CX_1 = R\!\uparrow_{F_e}$ 是树 $\mathrm{Rep}_F(-, \Lambda)$ 的胞腔链。由 Frobenius 互反律（沿轨道范畴的诱导-限制伴随），$\mathrm{Nat}_{\mathcal{O}_C(F)}(R\!\uparrow_{F_x},\, M\!\downarrow) = M(F_x)$ —— Mackey functor 在子系统 $F_x$ 上的值。$f^*$ 化为
...
这正是分摊 $(F_1, F_e, F_2)$ 上 $M$ 的 Mayer-Vietoris 余核。粘合问题应有的障碍：$M(F_e)$ 里某个元素是猜想的反例，当且仅当它*不是*分摊两侧元素的限制之差。
...
对任意 Mackey functor —— 比如 Burnside functor $B$ —— *没有* transfer 分裂论证可用，因为 $B$ 不在 stable elements 机器的像里。所以 Mayer-Vietoris 余核原则上可以非零，如果非零，就是个 Díaz-Park 猜想在这家族上失败、但*上同调* sharpness 没失败的具体见证。
...
1. 写出 $B(F_1)$，$B(F_2)$，$B(F_e)$。
2. 写出两个限制映射。
3. 取它们在 $B(F_e)$ 里的联合像。
4. 商掉。

如果在多项式家族上余核是零，这是猜想以我目前不知道的方式延伸到上同调之外的证据，多项式情形成为研究*为什么*的试验台。如果余核非零，据我所知这是有史以来第一个被显式展示的、Díaz-Park sharpness 的非上同调障碍。
...
余核如果是零，下一个问题是*为什么* —— 是不是有个我没注意到的、关于 Burnside 在有限群分摊上的 Mayer-Vietoris 事实？余核如果非零，下一个问题是*哪个元素*是障碍，它在 fusion 系统结构里是什么意思。
