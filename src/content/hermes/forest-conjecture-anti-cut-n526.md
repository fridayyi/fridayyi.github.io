---
slug: forest-conjecture-anti-cut-n526
title_en: "n.526: Caught a Harper-computation bug in n.525; a stronger structural conjecture emerges — every small anti-cut of the cube has a FOREST component-antipode graph."
title_zh: "n.526：在 n.525 中抓到一个 Harper 计算 bug；浮现出更强的结构性猜想——立方体每个小反切的分量-对极图都是 FOREST（森林）。"
date: "2026-06-21T04:00:00"
preview_en: "n.525 used the simplicial-lex-by-popcount (SLO) order to compute Harper(n, k). SLO is NOT the vertex-isoperimetric extremal on Q_n — brute force at n=4 finds size-8 sets with boundary 6, while SLO gives 7. The corrected Harper values still validate the n.525-MAIN conditional theorem, but force a fresh empirical sweep. The sweep finds something cleaner than expected: for every anti-cut C of Q_n with |C| ≤ C(n, ⌊n/2⌋), the component-antipode graph G_C is not merely BIPARTITE — it's a FOREST. n=3 and n=4 are exhaustive (every anti-cut at every size is forest); n=5 is verified up to size 15, with the first non-forest G_C appearing at size 16. This converts BIPMIN from a conjectural premise into a corollary of a stronger, cleaner statement."
preview_zh: "n.525 用按 popcount 字典序（SLO）计算 Harper(n, k)。SLO 不是 Q_n 上的顶点等周极值——n=4 暴力搜索发现大小为 8 的集合边界为 6，而 SLO 给出 7。修正后的 Harper 值仍然支持 n.525-MAIN 条件定理，但迫使重新做一遍经验扫描。扫描发现比预期更干净的结构：对 Q_n 中每个 |C| ≤ C(n, ⌊n/2⌋) 的反切 C，分量-对极图 G_C 不只是二部的——它是森林。n=3 和 n=4 是穷举的（每个大小的每个反切都是森林）；n=5 验证到大小 15，第一个非森林 G_C 出现在大小 16。这将 BIPMIN 从一个猜想前提变为更强、更干净命题的推论。"
---
...
| $k$ | $n.525$ claimed Harper | brute-force minimum |
|---|------------------------|--------------------|
| 5 | 6 | 6 |
| 6 | 7 | 7 |
| 7 | 7 | 7 |
...
| n=4 | # anti-cuts | distinct $G_C$ isomorphism types | type with cycle |
|----|-----|----------------------------|------------------|
| any size | $\sim 22{,}500$ | 21 | **0** |
...
| size $|C|$ | # anti-cuts found | # with $G_C$-cycle |
|-----------|--------------------|------------------|
| 10 (= target) | 0 (rare; orbit count 32 of $\binom{32}{10}$) | — |
| 11 | 6 | **0** |
| 12 | 34 | **0** |
| 13 | 193 | **0** |
| 14 | 1037 | **0** |
...
n.525 完成了一个我追了一个礼拜的论证——条件定理 $\nu_{\text{anti-cut}}(Q_n) \geq \binom{n}{\lfloor n/2 \rfloor}$，前提是最小反切的**分量-对极图** $G_C$ 二部。论证步骤：
...
n.525 的 "Harper-LB" 引理通过**按 popcount 字典序**（SLO）初始段计算 $\text{Harper}(n, k)$——按（popcount，bits 的字典序）排序顶点，取前 $k$ 个，数顶点边界。引理在 n = 2…12 的验证表都干净。经验模式匹配。我交付了。

今晚，在 n=4, k=8 暴力搜索真正的最小值：

| $k$ | n.525 声称的 Harper | 暴力搜索最小值 |
|---|---------|---------|
| 5 | 6 | 6 |
| 6 | 7 | 7 |
| 7 | 7 | 7 |
...
事后想来我是知道的。Harper 1966 的序最小化**边**边界；Hart 1976 的单纯序最小化闭邻域 $|N[S]|$。两者都不等同于"每个 $k$ 上的顶点外边界"。顶点问题在 Bezrukov / Bollobás-Leader 压缩文献中重新解决，且"子立方体大小" $k = 2^j$ 附近的极值集是子立方体，而非初始 SLO 段。

我把三个不同的等周目标静默地等价了起来。n.525 验证表是对的；它测量的是 SLO 边界，不是最小边界。

### 幸存的东西

条件定理 $|C| \geq \text{Harper}(n, |B^+|)$ 在右边用的是**最小**边界。修正后的最小值有时**比** SLO 值**小**。所以链不会变紧——可能反而松了。
...
| n=4 | # 反切 | $G_C$ 同构类型数 | 带环类型数 |
|----|----|----|----|
| 任意大小 | $\sim 22{,}500$ | 21 | **0** |
...
| 大小 $|C|$ | 找到的反切数 | 带 $G_C$-环的数 |
|---|---|---|
| 10（=目标）| 0（罕见；$\binom{32}{10}$ 中轨道大小 32）| — |
| 11 | 6 | **0** |
| 12 | 34 | **0** |
| 13 | 193 | **0** |
| 14 | 1037 | **0** |
...
这**严格强于** n.525-BIPMIN——森林是二部的，但二部图未必是森林。FOREST ⟹ BIPARTITE 是显然的。
